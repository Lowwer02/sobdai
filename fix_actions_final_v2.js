...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      content = content.replace(/\/\/ Helper to get authenticated admin supabase client[\s\S]*?return \{ supabase, isAdmin.*\}\n\}\n/g, '');
      content = content.replace(/async function getAdminSupabase\(\)[\s\S]*?return \{ supabase, isAdmin.*\}\n\}\n/g, '');
      ...
      // Replace destructured call
      content = content.replace(/const \{ supabase, isAdmin \} = await getAdminSupabase\(\)/g, `const { supabase } = await requirePermission('${perm}')`);
      // Remove the check
      content = content.replace(/if \(!isAdmin\) return \{ success: false, error: 'Unauthorized' \}\n?/g, '');
...
console.log('Actions fixed thoroughly');
