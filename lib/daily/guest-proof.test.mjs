...
import {
  ...
  GUEST_DAILY_PROOF_SECRET_MIN_BYTES,
  normalizeGuestCompletionAnswers,
  resolveDailyGuestProofSecret,
  validateDailyGuestProofSecret,
...
const secret = 'test-only-daily-proof-secret-with-32-bytes'
const issuedAt = Date.parse('2026-09-03T03:00:00.000Z')

test('the dedicated proof secret must be present and at least 32 bytes', () => {
  assert.equal(GUEST_DAILY_PROOF_SECRET_MIN_BYTES, 32)
  assert.throws(() => validateDailyGuestProofSecret(undefined), /DAILY_GUEST_PROOF_SECRET/)
  assert.throws(() => validateDailyGuestProofSecret('short-secret'), /at least 32 bytes/)
  assert.throws(() => validateDailyGuestProofSecret(` ${'a'.repeat(32)}`), /at least 32 bytes/)
  assert.equal(validateDailyGuestProofSecret(secret), secret)
  assert.equal(resolveDailyGuestProofSecret({ DAILY_GUEST_PROOF_SECRET: secret }), secret)
  assert.throws(
    () => resolveDailyGuestProofSecret({ SUPABASE_SERVICE_ROLE_KEY: secret }),
    /DAILY_GUEST_PROOF_SECRET/,
  )
})
