...
function verifies_runFilters_admits_wholly_absent_pattern_column(): void {
  // Every row has questionPattern === null. This must NOT fatal now.
...
function verifies_pattern_classifier_empty_throws() {
  assert.throws(
    () => classifyPatternAvailability([]),
    (err: any) => err.message.includes('precondition failed')
  )
}
...
const tests: Array<{ name: string; fn: () => void }> = [
  ...
  // Phase 1C-B Structural Pool & Exclusion Independence
