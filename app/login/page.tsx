'use client'

import { createClient } from '@/lib/supabase/client'
import { useSearchParams } from 'next/navigation'
import { Suspense } from 'react'

function GoogleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
    </svg>
  )
}

function LoginContent() {
  const searchParams = useSearchParams()
  const redirect = searchParams.get('redirect') ?? '/'
  const error = searchParams.get('error')
  const supabase = createClient()

  const handleGoogleLogin = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(redirect)}`,
      },
    })
  }

  return (
    <div style={{ maxWidth: '400px', margin: '80px auto', padding: '0 20px' }}>
      <div className="card" style={{ padding: '48px 36px', textAlign: 'center' }}>
        {/* Logo */}
        <div
          className="font-display"
          style={{ fontSize: '28px', marginBottom: '8px', color: 'var(--gold-light)' }}
        >
          สอบได้
        </div>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '36px' }}>
          เข้าสู่ระบบเพื่อบันทึกผลและเข้าถึงข้อสอบ
        </p>

        {error && (
          <div
            style={{
              background: 'var(--wrong-bg)',
              border: '1px solid rgba(224, 92, 92, 0.25)',
              borderRadius: 10,
              padding: '10px 14px',
              marginBottom: '20px',
              fontSize: '13.5px',
              color: 'var(--wrong)',
            }}
          >
            เข้าสู่ระบบไม่สำเร็จ กรุณาลองใหม่
          </div>
        )}

        <button
          onClick={handleGoogleLogin}
          id="btn-google-login"
          style={{
            width: '100%',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            gap: '10px',
            padding: '13px 20px',
            background: 'var(--bg-input)',
            border: '1.5px solid var(--border)',
            borderRadius: 12,
            color: 'var(--text-primary)',
            fontSize: '15px',
            cursor: 'pointer',
            fontFamily: "'Sarabun', sans-serif",
            transition: 'all 0.2s',
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.borderColor = 'var(--border-hover)'
            e.currentTarget.style.background = 'var(--gold-tint)'
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.borderColor = 'var(--border)'
            e.currentTarget.style.background = 'var(--bg-input)'
          }}
        >
          <GoogleIcon />
          เข้าสู่ระบบด้วย Google
        </button>

        <p
          style={{
            marginTop: '24px',
            fontSize: '12px',
            color: 'var(--text-faint)',
            lineHeight: 1.65,
          }}
        >
          สามารถเข้าชมเว็บและทดลองทำข้อสอบได้โดยไม่ต้องเข้าสู่ระบบ
          <br />
          เข้าสู่ระบบเพื่อซื้อข้อสอบและบันทึกผลคะแนน
        </p>
      </div>
    </div>
  )
}

export default function LoginPage() {
  return (
    <Suspense>
      <LoginContent />
    </Suspense>
  )
}
