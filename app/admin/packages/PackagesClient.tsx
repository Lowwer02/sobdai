...
import { Plus, Search, Filter, Edit, Copy, Archive, Loader2 } from 'lucide-react'
import ConfirmDialog from '@/components/admin/ConfirmDialog'
import { toastEvent } from '@/hooks/useToast'
import { deletePackageAction } from './actions'
import { useRouter } from 'next/navigation'

export default function PackagesClient({ packages, currentUserRole }: { packages: any[], currentUserRole: string }) {
  const router = useRouter()
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean, pkg: any }>({ isOpen: false, pkg: null })
  const [isDeleting, setIsDeleting] = useState(false)

  const confirmDelete = async () => {
    if (!deleteModal.pkg) return
    setIsDeleting(true)
    const res = await deletePackageAction(deleteModal.pkg.id)
    setIsDeleting(false)
    
    if (res.success) {
      toastEvent('ลบแพ็กเกจเรียบร้อยแล้ว')
  ...
  return (
    <div className="space-y-6">
            ...
            <tbody className="divide-y divide-[rgba(255,255,255,0.02)]">
              {packages.length === 0 ? (
                ...
                <tr key={pkg.id} className="hover:bg-[#D4AF37]/[0.02] transition-colors">
                  ...
                  <td className="p-4">
                    <div className="text-[#F5E9D6] text-sm whitespace-nowrap">{pkg.total_questions?.toLocaleString() || 0} Qs</div>
                    <div className="text-[#A1866B] text-xs whitespace-nowrap">{pkg.total_exam_sets || 0} Sets</div>
                  </td>
      ...
      <ConfirmDialog
        ...
        onClose={() => !isDeleting && setDeleteModal({ isOpen: false, pkg: null })}
        onConfirm={confirmDelete}
        title="ลบแพ็กเกจ"
        description={
          <div className="space-y-2 text-[#F5E9D6]">
            <div>คุณกำลังจะลบแพ็กเกจ: <span className="text-red-400 font-medium">{deleteModal.pkg?.name}</span></div>
            <p className="text-red-400 text-xs">การลบแพ็กเกจจะลบข้อมูลที่เกี่ยวข้องทั้งหมดแบบถาวร ไม่สามารถกู้คืนได้</p>
          </div>
        }
        confirmText="ลบแพ็กเกจถาวร"
        cancelText="ยกเลิก"
        isDestructive={true}
