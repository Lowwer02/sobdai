...
import type { PublicPositionPackage } from '@/lib/positions-public'
import styles from '@/app/positions/[slug]/positions.module.css'

interface PositionPackagesSectionProps {
  items: readonly PublicPositionPackage[]
}
...
function organizationLabel(item: PublicPositionPackage): string {
  return item.organization?.short_name || item.organization?.name || 'แพ็กเกจสำหรับตำแหน่งนี้'
}

function packageLogoUrl(item: PublicPositionPackage): string | null {
  const value = item.logo_url?.trim()
  return value || null
}

export default function PositionPackagesSection({ items }: PositionPackagesSectionProps) {
  if (items.length === 0) return null

  return (
    <section
      id="packages"
      aria-labelledby="position-packages-heading"
    ...
    >
      <header className={styles.relatedSectionHeader}>
        ...
        <h2 id="position-packages-heading" className={styles.relatedSectionHeading}>
          เตรียมสอบตำแหน่งนี้
        </h2>
        <p className={styles.relatedSectionDescription}>
          เลือกสนามสอบและแพ็กเกจที่ตรงกับหน่วยงานที่คุณเตรียมสอบ
        </p>
      </header>
          ...
          return (
            <li key={item.id} className={styles.packageCardItem}>
              <Link href={`/package/${encodeURIComponent(item.slug)}`} className={styles.packageCard}>
                ...
                <span className={styles.packageOrganization}>{organizationLabel(item)}</span>
