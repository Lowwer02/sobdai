...
**Date:** 2026-07-27
**Scope:** Recommendation Engine (E-3) — the evaluation, scoring, deduplication, ranking, business-rule, and recommendation-assembly layer that consumes RecommendationCandidate[] from E-2 (Candidate Discovery) and produces an immutable RecommendationSet.
...
The Recommendation Engine is a **pure business engine** that transforms a flat list of discovered candidates into a deduplicated, ranked, business-rule-filtered RecommendationSet.

```
   RecommendationCandidate[]  ──────►  ┌─────────────────────────┐  ──────►  RecommendationSet
   (from E-2 Discovery)                │ Recommendation Engine    │           (for application service)
...
- Assigns a numeric score to each evaluated candidate.
- Deduplicates (same content discovered via different signals → pick the best).
- Ranks the deduplicated candidate set by score.
- Applies business rules (caps per category, minimum evidence, freshness).
- Assembles the final `RecommendationSet` with immutable `EngineRecommendation` objects.
- Assigns final sequential priority during assembly only.
...
- ❌ Enrich targets or transform UI-specific DTOs (Application Service responsibility).
...
Question Bank → Exam Set → Assessment Runtime → Assessment Outcome
    → Assessment Analytics → Recommendation Candidate Discovery (E-2)
        → Recommendation Engine (E-3 — this spec)
            → RecommendationSet → Application Service / Target Resolver → UI
```
...
fetchMyAnalytics()
    ...
    → RecommendationSet
    → target enrichment / UI DTO transformation              [Application Service]
    → UI
```

**Migration path:** E-3 is introduced as a new module (`lib/recommendation/engine.ts`). The existing `recommend()` function in `lib/assessment/recommendation.ts` becomes a **legacy adapter** (or is eventually retired). The server action `fetchMyRecommendations()` is updated through an Application Service that calls `discoverCandidates` → `engine.rank` instead of `recommend` directly. Any target enrichment or UI-specific DTO transformation happens after E-3; the Engine contract remains immutable and UI-agnostic.
...
### 4.2 Recommendation (output — assembled by E-3)

The Engine produces a `RecommendationSet` of immutable `EngineRecommendation` objects. UI-specific DTO transformation is outside E-3:
...
interface EngineRecommendation {
  ...
  /** Human-readable learner-facing title (Thai). */
  readonly title: string
  /** Human-readable learner-facing reason (Thai). */
...
│                                                                      │
│  2. Deduplication                                                   │
│     Group by content identity (content kind + contentId).           │
│     Keep the highest-scoring candidate per content identity.        │
│     Record deduped count for stats.                                 │
│     → DeduplicatedCandidate[] (unique; priority NOT assigned)       │
│                                                                      │
│  3. Ranking                                                         │
...
│       Attach candidate content reference as target.                 │
│       Carry scoring breakdown + evidence + candidateId.             │
│       ASSIGN sequential priority (1, 2, 3, ...) at the final        │
...
**Frozen refinements:** Deduplication runs before final ranking, so ranking
operates on the unique candidate set. Priority is NOT assigned during Ranking.
Ranking determines ordering by score only. Sequential priority (1, 2, 3, ...)
is assigned during Assembly — the final Engine output boundary — AFTER dedup,
ranking, business rules, and filtering. Assembly produces immutable
`EngineRecommendation` objects only; target enrichment and UI DTO transformation
remain outside the Engine in the Application Service layer.
...
├── score: number                    (0..100)
├── title: string                    (Thai learner-facing text)
├── reason: string                   (Thai learner-facing explanation)
├── target: RecommendationTarget     (candidate content reference; may be null)
...
├── subject: string | null           (trace/grouping metadata)
├── topic: string | null
...
Future AI integration = implement `AIScoringStrategy` that calls an LLM with the candidate's metadata + evidence and returns a score. The pipeline (evaluate → score → dedup → rank → rules → assemble) is unchanged.

---

## 8. Ranking Architecture

After scoring and deduplication, candidates are sorted by `score` descending. Ties are broken by:
1. Signal name (alphabetical — no business priority, structural determinism only).
2. Content ID (stable, unique).

This produces a fully deterministic order over the final unique candidate set: same candidates → same ranking.

**Priority is NOT assigned here.** Ranking determines ordering only — it produces a score-sorted sequence. Sequential priority (1, 2, 3, ...) is assigned during Assembly (§11), the final Engine output boundary, AFTER deduplication and business-rule filtering. This separation ensures:
- Intermediate stages (dedup, ranking, rules) operate purely on scores — no presentation concerns leak in.
...
**Rule:** If the same content identity appears via multiple signals (e.g., a Summary discovered via both `weak_subject` and `weak_topic`), keep only the one with the highest score.

**Content identity:** `candidate.content.kind + ":" + candidate.content.contentId`.

This prevents accidental collapse of different content types that happen to use the same underlying ID value.

**Implementation:**
```
Group candidates by content.kind + content.contentId
For each group: pick the candidate with the highest score
Record (originalCount - uniqueCount) as dedupedCount for stats
```

Deduplication happens AFTER scoring and BEFORE final ranking. This keeps winner selection score-based while ensuring the ranking stage operates on the final unique candidate set rather than temporary duplicates. This is the ONE place the Engine "chooses a winner" — and it chooses by score, not by hardcoded priority.

---

## 10. Business Rule Layer

Business rules are applied as a **filter stage** after deduplication and ranking. Each rule can DROP candidates or REORDER candidates. Rules are pluggable (array of `BusinessRule` functions):
...
The final stage converts each surviving `ScoredCandidate` into an immutable `EngineRecommendation`. This is where **priority is assigned** — it belongs to the final Engine output, not to ranking.

| Field | Source |
|---|---|
| `id` | Candidate's `id` (already deterministic) |
| `category` | Mapped from `candidate.signal` via a signal→category map |
| `priority` | **Assigned HERE** — sequential (1, 2, 3, ...) in the final score-sorted order AFTER dedup + ranking + business rules. This is the ONLY place priority exists; ranking (§8) produces ordering only. |
...
| `target` | Built from `candidate.content` as an engine-level content reference (kind/contentId/slug/packageId); DB-only enrichment fields remain unresolved. |
...
Ranking determines the score-ordering of unique candidates. But the FINAL display
order depends on business rules, which may filter or reorder candidates.
Assigning priority during ranking would produce gaps
...
**Assembly boundary refinement:** Assembly produces `EngineRecommendation`
objects only. It does not resolve package slugs, ownership, URLs, UI-specific
DTOs, or presentation-only transformations. Those responsibilities live in the
Application Service / Target Resolver after E-3.
...
```
 Server Action            Engine              ScoringStrategy       BusinessRules
      ...
      │                      │◄── ScoringBreakdown──│                     │
      │                      │                      │                     │
      │                      │  2. Dedup by content identity              │
      │                      │     (kind + contentId, keep best score)    │
      │                      │  3. Rank unique candidates (sort desc)     │
      │                      │                      │                     │
      │                      │  4. Business Rules ──┼────────────────────►│
...
**Reason:** Clean separation (Architecture Spec: "Engine must not know Summary Database, Question Database, or Supabase"). Makes the Engine fully testable with synthetic candidates. Enrichment and UI DTO transformation happen AFTER the Engine in the Application Service / Target Resolver, not inside it.
...
**Reason:** The user requirement: "Engine must support Rule-based → Hybrid → AI-assisted → ML without changing architecture." Injecting the strategy means swapping scoring intelligence is a one-line change at the call site. The pipeline (evaluate → score → dedup → rank → rules → assemble) never changes.
...
**Decision:** When the same content identity appears via multiple signals, the candidate with the highest SCORE wins. Deduplication runs before final ranking.
**Reason:** This is the Engine's core decision-making responsibility. Score-based dedup means the scoring strategy controls which signal wins — not a hardcoded `weak_subject > weak_topic` ordering. Running dedup before ranking ensures ranking operates on the final unique candidate set. If the strategy changes (AI scoring), the dedup winner changes accordingly.
...
### D6: Assembly Produces Engine Objects, Not UI DTOs
**Decision:** `EngineRecommendation` is the immutable E-3 output object. It carries engine-level fields (`category`, `priority`, `title`, `reason`, `target`, `score`, `scoringBreakdown`, `candidateId`) but does not resolve UI URLs, ownership, package slugs, or route-specific DTOs.
**Reason:** The Engine remains a pure decision layer. The Application Service owns target enrichment and any compatibility transformation for the existing UI contract.
